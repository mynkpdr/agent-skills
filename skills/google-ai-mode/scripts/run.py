...
def ensure_venv():
    """Ensure virtual environment exists and has a working interpreter"""
    skill_dir = Path(__file__).parent.parent
    setup_script = skill_dir / "scripts" / "setup_environment.py"
    venv_python = get_venv_python()

    # Check for a working interpreter, not just the directory. A directory
    # that exists but has no python binary (e.g. setup was interrupted
    # partway through) would otherwise be silently treated as "already set
    # up" forever, only to fail confusingly later when we try to run it.
