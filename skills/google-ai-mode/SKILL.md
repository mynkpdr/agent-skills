...
```
- Saves detailed logs to `~/.cache/google-ai-mode-skill/logs/search_YYYY-MM-DD_HH-MM-SS.log` (macOS: `~/Library/Caches/google-ai-mode-skill/logs/`, Windows: `%LOCALAPPDATA%\google-ai-mode-skill\logs\`)
...
**`--save`** - Save results outside the skill folder
```bash
python scripts/run.py search.py --query "..." --save
```
- Saves markdown to `~/.cache/google-ai-mode-skill/results/YYYY-MM-DD_HH-MM-SS_Query_Name.md` (macOS: `~/Library/Caches/google-ai-mode-skill/results/`, Windows: `%LOCALAPPDATA%\google-ai-mode-skill\results\`)
- Timestamped filename for organized storage
- Lives outside the skill's git-managed folder, so results survive a skill update/reinstall
...
| Need detailed logs | Use `--debug` flag - log saved to `~/.cache/google-ai-mode-skill/logs/` |
